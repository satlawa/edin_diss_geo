...
class DataTools(object):
    ...
    def align_rasters(self, path_in_rast, path_in_proj, path_out):
        '''
        aligns pixels between two rasters
        path_in_rast = path to raster that is going to be aligned
        path_in_proj = path to raster that serves as the source for the alignment
        path_out     = path to the aligned output raster
        '''
        # open rasters
        ds1 = gdal.Open(path_in_rast)
        ds2 = gdal.Open(path_in_proj)

        # get GeoTransform from raster 'path_in_proj'
        geotransform = ds2.GetGeoTransform()
        # align raster 'path_in_rast' to raster 'path_in_proj'
        gdal.Warp(path_out, ds1, targetAlignedPixels = True, xRes = geotransform[1], yRes = geotransform[1])


    def clip_raster(self, raster_path, shp_path, path_out):
        result = gdal.Warp(path_out,
                   raster_path,
    ...
    def write_raster_array(self, clip, path_out, gt2, proj):
        '''
        writes numpy array to raster
        path_out     = path to the aligned output raster
        '''
        if len(clip.shape) == 2:
        ...
        dataset = driver.Create(path_out, x_pixels, y_pixels, channels, data_type)
    ...
    def clip_raster_array(self, raster_path, features_path, gt=None, nodata=-9999):
        '''
        ...
        def array_to_image(a):
            ...
            '''
            img = Image.fromstring('L',(a.shape[1], a.shape[0]),
                (a.astype('b')).tostring())
            return img
